...
with sigmoid curves. NOW INCLUDING CDTM nodes to show when alumni attended CDTM.
"""
...
def extract_paths(alumni_data: List[Dict], schools_data: Dict) -> List[Dict]:
    ...
    for person in alumni_data:
        ...
        # Strategy: Insert CDTM after Bachelor's/Diploma, or in the middle if no clear Bachelor's
        cdtm_level = None
        ...
        # If still no position, put CDTM at the beginning if they have degrees
...
def define_stations() -> Dict[str, Tuple[float, float]]:
    ...
    NOW INCLUDING ONE CENTRAL CDTM NODE!
    """
    stations = {
...
def plot_education_flows(paths: List[Dict], output_file: str = 'education_flow_viz.png'):
    ...
    print(f"CDTM connections: {cdtm_count}")
    ...
    for station_name, (sx, sy) in stations.items():
        ...
        # White/colored background circle
    ...
    ax.set_ylim(-0.5, 8)
    ax.axis('off')
    ax.set_title('CDTM Alumni Education Pathways (Including CDTM)',
...
def print_statistics(paths: List[Dict]):
    ...
    print("\n" + "="*60)
    print("EDUCATION PATH STATISTICS (WITH CDTM)")
    print("="*60)
    ...
    print(f"Paths including CDTM: {paths_with_cdtm} ({paths_with_cdtm/len(paths)*100:.1f}%)")
    ...
    # CDTM timing
    cdtm_bachelor_level = sum(1 for p in paths
                              for n in p['nodes']
                              if n.get('is_cdtm') and n.get('cdtm_level') == "Bachelor's Level")
    cdtm_master_level = sum(1 for p in paths
                           for n in p['nodes']
                           if n.get('is_cdtm') and n.get('cdtm_level') == "Master's Level")

    print("\n--- CDTM Timing ---")
    print(f"  During Bachelor's level: {cdtm_bachelor_level}")
    print(f"  During Master's level: {cdtm_master_level}")

    # Path length distribution
    path_lengths = [len(p['nodes']) for p in paths]
    print("\n--- Path Lengths (including CDTM) ---")
...
def main():
    ...
    print("\nExtracting education paths (including CDTM)...")
    ...
    print("\nCreating flow visualization with CDTM nodes...")
